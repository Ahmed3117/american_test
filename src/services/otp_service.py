...
class OTPService:
    ...
    def generate_otp(self, length=None):
        ...
        """
        fixed_otp = os.getenv("OTP_FIXED_CODE")
        if fixed_otp:
            logger.info("Using fixed OTP from OTP_FIXED_CODE for local/testing flow")
            return str(fixed_otp)
    ...
    def verify_otp(self, phone_number, otp_code, purpose='verification', mark_as_used=True):
        ...
        try:
            with transaction.atomic():
                # Find the most recent unused OTP for this phone/purpose with row-level lock
                otp_record = OTP.objects.select_for_update().filter(
                    phone_number=phone_number,
                    purpose=purpose,
                    is_used=False,
                    is_verified=False
