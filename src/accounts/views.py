...
from .models import DeletedUserArchive, User, UserDevice
...
def signin_dashboard(request):
    """Signin endpoint for staff/superusers (dashboard)."""
    username = request.data.get('username')
    password = request.data.get('password')
...
class DeleteAccountView(APIView):
    ...
    def delete(self, request):
        ...
        if user.is_staff or user.is_superuser:
            return Response({'error': 'لا يمكن حذف حسابات المدير عبر هذا المسار.'}, status=status.HTTP_403_FORBIDDEN)
        ...
        logger.info(f"✅ [USER_DELETE] Archived user {username} - Archive ID: {archive.id}")
...
class UserDeleteAPIView(APIView):
    ...
    def delete(self, request, pk):
        ...
        with transaction.atomic():
            ...
            logger.info(f"✅ [USER_DELETE] Archived user {user.username}")
